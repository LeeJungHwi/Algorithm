...
int main()
{
	...
	int vAns = home, oAns = home;
	loop(i, home, n) loop(j, home, m) if (graph[i][j] == 'v' || graph[i][j] == 'o')
	{
		cp.push({ i, j });
		int vCnt = home, oCnt = home;
		cond(graph[i][j] == 'v', vCnt++, oCnt++);
		graph[i][j] = '#';
		while (!cp.empty())
		{
			...
			loop(k, home, 4)
			{
				...
				if (graph[ci][cj] == 'o') oCnt++;
				cp.push({ ci, cj });
				graph[ci][cj] = '#';
			}
		}
		cond(vCnt >= oCnt, vAns += vCnt, oAns += oCnt);
	}
	scp(oAns); elp(vAns);
