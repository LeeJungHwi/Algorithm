...
int n, m, k, ans = MIN, sum; dir;

void DFS(int L, int s, vector<vector<bool>> &vis, vector<vector<int>> &graph)
{
	if (L == k)
	{
		ans = max(ans, sum);
	...
	else
	{
		loop(i, s, n * m)
		{
			...
			loop(j, home, 4)
			{
				...
				if (cii < home || cjj < home || cii >= n || cjj >= m) continue;
				if (vis[cii][cjj])
				{
					isAdja = true;
					break;
