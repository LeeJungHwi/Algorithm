...
{
	// 첫 가로등이 0을 비출 수 있는지 체크
	if (v[home] > s) return false;

	// 각 가로등 사이의 거리를 비출 수 있는지 체크
	loop(i, home, v.size() - 1)
	{
		if (v[i + 1] - v[i] > s * 2)
		{
			return false;
...
int main()
{
	...
	// lo => 0
	// hi => n
	// s로 길이 n을 비출 수 있으면 가능한 경우
	cin >> n >> m;
	vec(int, v, m);
	loop(i, home, m) cin >> v[i];

	int l = home, h = n, ans = n;
