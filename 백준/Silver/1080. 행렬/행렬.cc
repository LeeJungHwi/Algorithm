...
int main()
{
	...
	// 3 by 3을 뒤집어서 행렬 A를 B로 만들기
	// 이미 확정된 부분은 다시 뒤집지 않음 => 행렬을 탐색하면서 A와 B가 다르다면 뒤집기
	// 다 끝나고 A와 B가 다르다면 불가능
	int n, m; cin >> n >> m;
	mat(int, A, n, m);
	mat(int, B, n, m);
	...
	int ans = home;
	loop(i, home, n - 2) loop(j, home, m - 2)
	{
		// i, j에서 A와 B가 다르면 뒤집어서 A[i][j]를 확정시킴
		if (A[i][j] != B[i][j])
		{
			loop(k, i, i + 3) loop(l, j, j + 3) A[k][l] = !A[k][l];
			ans++;
		}
	}
