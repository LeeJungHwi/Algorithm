...
void DFS(int L, int bitmask, vector<int> &v)
{
	if (L == n)
	{
		// 여기 까지 왔으면 중량 500이상을 유지한 경우이므로 카운팅
		ans++;
	}
	else
	{
...
int main()
{
	...
	// 하루마다 중량이 k만큼 감소
	// 하루 1개씩 키트를 사용함
	// 키트를 사용하면 해당 키트의 값만큼 중량이 증가
	// 1일 ~ N일 까지 중량이 500 이상이 유지 되는 경우의 수

	// 각 날에 어떤 키트를 사용할지 => 비중복순열
	// 중간에 중량이 500 미만이 되면 X
