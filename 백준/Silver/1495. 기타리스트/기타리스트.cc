...
int main()
{
	...
	// dp[i][j] => i번째 곡에서 j볼륨이 가능한지 체크
	// dp[0][s] => true
	// dp[1]의 가능한 j => dp[0]의 가능한 j를 보면서 v[1]을 더하거나 뺐을 때 0과 m사이면 가능한 경우로 체크
	// 마지막 곡에서 가능한 볼륨 중 최댓값 업데이트
	int n, s, m; cin >> n >> s >> m;
	vec(int, v, n + 1);
	...
	loop(i, 1, n + 1)
	{
		loop(j, home, m + 1)
		{
			...
			// 이전 곡에서 가능한 볼륨들을 보면서 v[i]를 더하거나 뺐을 때 0과 m사이면 가능한 경우로 체크
			if (j + v[i] <= m) dp[i][j + v[i]] = true;
			if (j - v[i] >= home) dp[i][j - v[i]] = true;
		}
	}
