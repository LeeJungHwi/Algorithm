...
int main()
{
	...
	loop(i, home, n)
	{
		...
		int hour = stoi(time.substr(home, 2));
		int minute = stoi(time.substr(3, 2)) + hour * 60;

		feeMap[name] += minute;
	}
	mloop(it, feeMap)
	{
		bool isRemain = it->rhs % 50;
		int fee = it->rhs <= 100 ? 10 : 10 + (it->rhs - 100) / 50 * 3 + isRemain * 3;
		it->rhs = fee;
	}
	
	map<int, vector<string>> nameMap;
	mloop(it, feeMap) nameMap[it->rhs].push_back(it->lhs);
	mrloop(it, nameMap)
	{
		sort(all(it->rhs));
