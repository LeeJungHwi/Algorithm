...
void DFS(int L, int s, int bitmask, vector<int>& v, map<int, int> &ans)
{
	if (L == m)
	{
...
int main()
{
	...
	// n마리 소 중 m마리 비중복조합 뽑아서 몸무게 합이 소수면 ans에 저장
	cin >> n >> m;
	vec(int, v, n);
	loop(i, home, n) cin >> v[i];
	
	map<int, int> ans;
	DFS(home, home, home, v, ans);
	
	if (ans.empty()) { elp(-1); return home; }
	mloop(it, ans) scp(it->lhs);
