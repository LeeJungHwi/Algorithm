#include <bits/stdc++.h>
using namespace std;

#define home 0

#ifdef ONLINE_JUDGE
#define init ios_base::sync_with_stdio(home); cin.tie(home)
#else
#define init ios_base::sync_with_stdio(home); cin.tie(home); ifstream cin("input.txt")
#endif

#define ll long long
#define ld long double

#define pii pair<int, int>
#define piii pair<int, pii>
#define pll pair<ll, ll>
#define plll pair<ll, pll>

#define loop(v, s, e) for(int v = (s); v < (e); v++)
#define rloop(v, s, e) for(int v = (s); v > (e); v--)
#define mloop(v, a) for(auto v = (a).begin(); v != (a).end(); v++)
#define mrloop(v, a) for(auto v = (a).rbegin(); v != (a).rend(); v++)

#define p(a) cout << (a)
#define elp(a) cout << (a) << '\n'
#define scp(a) cout << (a) << ' '

#define tvec(t, v) vector<t> v
#define vec(t, v, r) vector<t> v((r))
#define ivec(t, v, r, i) vector<t> v((r), i)
#define gmat(t, v, r) vector<vector<t> > v((r))
#define mat(t, v, r, c) vector<vector<t> > v((r), vector<t>((c)))
#define imat(t, v, r, c, i) vector<vector<t> > v((r), vector<t>((c), i))
#define smat(t, v, r, c, s) vector<vector<vector<t> > > v((r), vector<vector<t>>((c), vector<t>((s))))
#define ismat(t, v, r, c, s, i) vector<vector<vector<t> > > v((r), vector<vector<t>>((c), vector<t>((s), i)))
#define ssmat(t, v, r, c, s1, s2) vector<vector<vector<vector<t> > > > v((r), vector<vector<vector<t>>>((c), vector<vector<t>>((s1), vector<t>((s2)))))
#define issmat(t, v, r, c, s1, s2, i) vector<vector<vector<vector<t> > > > v((r), vector<vector<vector<t>>>((c), vector<vector<t>>((s1), vector<t>((s2), i))))
#define sssmat(t, v, r, c, s1, s2, s3) vector<vector<vector<vector<vector<t> > > > > v((r), vector<vector<vector<vector<t>>>>((c), vector<vector<vector<t>>>((s1), vector<vector<t>>((s2), vector<t>((s3))))))
#define isssmat(t, v, r, c, s1, s2, s3, i) vector<vector<vector<vector<vector<t> > > > > v((r), vector<vector<vector<vector<t>>>>((c), vector<vector<vector<t>>>((s1), vector<vector<t>>((s2), vector<t>((s3), i)))))

#define dir vector<pii> cd = { {-1, home}, {1, home}, { home, -1 }, { home, 1 }, { -1, -1 }, { -1, 1 }, { 1, -1 }, { 1, 1 } }
#define kdir vector<pii> kcd = { {-1, -2}, {-2, -1}, { -2, 1 }, { -1, 2 }, { 1, -2 }, { 2, -1 }, { 1, 2 }, { 2, 1 } }
#define lhs first
#define rhs second

#define cond(c, t, f) ((c) ? (t) : (f))
#define all(a) (a).begin(), (a).end()
#define rall(a) (a).rbegin(), (a).rend()

const int MAX = 2147000000;
const int MIN = -2147000000;

bool comp(const piii & p1, const piii & p2)
{
	if (p1.rhs.lhs != p2.rhs.lhs) return p1.rhs.lhs < p2.rhs.lhs;
	return p1.rhs.rhs < p2.rhs.rhs;
}

// 강의실 2
int main()
{
	init;

	// 최소 강의실 개수
	int n; cin >> n;
	vec(piii, room, n); // (방 번호, 시작시간, 끝나는시간)
	loop(i, home, n) cin >> room[i].lhs >> room[i].rhs.lhs >> room[i].rhs.rhs;

	// 시작시간 오름차순
	// 끝나는시간 오름차순
	sort(room.begin(), room.end(), comp);

	priority_queue<pii, vector<pii>, greater<pii>> pq; // (끝나는시간, 강의번호)
	vec(int, roomNum, n); // 각 강의가 배정되는 강의실 번호
	loop(i, home, room.size())
	{
		int curLec = room[i].lhs;
		int curStart = room[i].rhs.lhs;
		int curEnd = room[i].rhs.rhs;

		// 기존 강의실중 가장빨리끝나는 강의의 시간이
		// 새롭게 시작할 강의의 시작시간 이하면 기존 강의실 사용
		if (!pq.empty() && curStart >= pq.top().lhs)
		{
			roomNum[curLec - 1] = roomNum[pq.top().rhs - 1];
			pq.pop();
		}
		// 아니면 새로운 강의실 사용
		else roomNum[curLec - 1] = pq.size() + 1;

		pq.push({ curEnd, curLec });
	}
	elp(pq.size());
	loop(i, home, n) elp(roomNum[i]);

	return home;
}