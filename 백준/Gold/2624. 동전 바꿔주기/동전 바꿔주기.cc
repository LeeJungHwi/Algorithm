...
int main()
{
	...
	int n, m; cin >> n >> m;
	vec(pii, coin, m);
	loop(i, home, m) cin >> coin[i].lhs >> coin[i].rhs;
	vec(int, dp, n + 1); // dp[i] => i원을 만드는 가지 수
	dp[home] = 1; // base => 아무 동전도 사용하지 않음

	// 모든 동전에 대해
	loop(i, home, m)
	{
		// 역방향 냅색
		rloop(j, n, coin[i].lhs - 1)
		{
			// 현재 동전을 1~최대 개수 까지 사용
			loop(k, 1, coin[i].rhs + 1)
			{
				// 이전 금액이 유효한 범위가 아니면 X
				if (j - coin[i].lhs * k < home) continue;
				
				// 이전 금액의 가지 수 누적
				dp[j] += dp[j - coin[i].lhs * k];
			}
		}
	}
	elp(dp[n]);
