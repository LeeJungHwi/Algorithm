...
int main()
{
	...
	// 정사각형 크기 1 ~ n
	// 누적합 => dp[i - 1][j] + dp[i][j - 1] - dp[i - 1][j - 1] + graph[i][j]
	// 부분합 => dp[i][j] - dp[i - k][j] - dp[i][j - k] + dp[i - k][j - k]
	...
	// 부분합 최댓값 업데이트
	int ans = MIN; 
	loop(k, 1, n + 1) loop(i, k, n + 1) loop(j, k, n + 1) ans = max(ans, dp[i][j] - dp[i - k][j] - dp[i][j - k] + dp[i - k][j - k]);
	elp(ans);
